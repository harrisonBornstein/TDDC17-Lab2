...
public class CustomGraphSearch implements SearchObject {
	 ...
	 */
	public ArrayList<SearchNode> search(Problem p, boolean returnExpansion) {
		...
		while(!frontier.isEmpty())
		{
			...
			of.addNodesToQueue(wrapStatesInNodes(n, p.getReachableStatesFrom(nodeState)), frontier);
			if (returnExpansion) {
			    Iterator it = frontier.l.iterator();
			     while (it.hasNext()) {
	...
	protected ArrayList<SearchNode> wrapStatesInNodes(SearchNode parentNode, ArrayList<GridPos> reachedStates) {
        ArrayList<SearchNode> retVal = new ArrayList<SearchNode>();

        for (int i = 0; i < reachedStates.size(); i++) {
            GridPos state = reachedStates.get(i);
	...
	private boolean shouldEndExpansion(Object state, SearchNode parent) {
        ...
        boolean stateSeenBefore = (!(explored.contains(state)));
        if (maxDepth > 0) { 
